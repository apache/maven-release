...
public abstract class AbstractReleaseTestCase extends PlexusJUnit4TestCase {
    ...
    protected void comparePomFiles(
            List<MavenProject> reactorProjects, String expectedFileSuffix, boolean normalizeLineEndings)
            throws IOException {
        for (MavenProject project : reactorProjects) {
                ...
                if ("${remoterepo}".equals(comparison.getControlDetails().getValue())
                    ...
                    return ComparisonResult.EQUAL;
                        ...
                        && "${project.build.directory}/site"
                                .equals(comparison.getTestDetails().getValue())) {
    ...
    private String getRemoteRepositoryURL() throws IOException {
        ...
        if (testFile.getAbsolutePath().equals(testFile.getCanonicalPath())) {
            return "file://"
                    + getTestFile("src/test/remote-repository")
                            .getAbsolutePath()
                            .replace('\\', '/');
        }
        return "file://"
                + getTestFile("src/test/remote-repository").getCanonicalPath().replace('\\', '/');
    }
     ...
     */
    private static final class SimpleReactorWorkspaceReader implements WorkspaceReader {
        ...
        public List<String> findVersions(org.eclipse.aether.artifact.Artifact artifact) {
            for (MavenProject mavenProject : reactorProjects) {
                if (Objects.equals(
                        artifact.toString(), mavenProject.getArtifact().toString())) {
